...
var MiniMap = /** @class */ (function () {
    function MiniMap(_a) {
        ...
        this.__lf = null;
        this.__container = null;//完整小地图的dom容器
        this.__miniMapWrap = null;//图的dom容器，小地图去掉header的核心部分
        this.__miniMapContainer = null;//__createMiniMap方法后的完整小地图的dom容器
        this.__lfMap = null;
        this.__viewport = null;//小地图的视窗，就那个可以拖动的框框
        this.__width = 150;
        this.__height = 220;
        ...
        this.__viewPortScale = 1;//所有节点的边界与minimap看板的边界的比例，及宽和高的比例的最小值
    ...
    MiniMap.prototype.__init = function () {
        var miniMapWrap = document.createElement('div');//document.createElement()是在对象中创建一个对象，要与appendChild() 或 insertBefore()方法联合使用
        ...
        this.__lfMap = new this.__LogicFlow({
            ...
            container: miniMapWrap,//图的 DOM 容器
            isSilentMode: true,
            stopZoomGraph: true,
    ...
    MiniMap.prototype.__createMiniMap = function (left, top) {
        var miniMapContainer = document.createElement('div');//整个小地图
        var miniMapWrap = this.__miniMapWrap;//取出小地图的核心部分
        ...
        this.__container.appendChild(miniMapContainer);//将整个小地图的dom容器保存给__container
        this.__miniMapWrap.appendChild(this.__viewport);//给小地图核心部分加上之前init时创建的视窗
        var header = document.createElement('div');//小地图的header
        header.className = 'lf-mini-map-header';
        header.innerText = '导航';
        miniMapContainer.appendChild(header);//给整个小地图加上header
        var close = document.createElement('span');//小地图的关闭按钮
        close.className = 'lf-mini-map-close';
        close.addEventListener('click', this.hide);//给关闭按钮绑定关闭事件
        miniMapContainer.appendChild(close);//给小地图完整dom容器加上关闭dom容器
    ...
    MiniMap.prototype.__setView = function () {
        ...
        var data = this.__resetData(this.__lf.getGraphRawData());//getGraphRawData获取流程绘图原始数据
        // 由于随时都会有新节点注册进来，需要同步将注册的
        var viewMap = this.__lf.viewMap;//当前幕布上的viewMap
        var modelMap = this.__lf.graphModel.modelMap;//graphModel是LogicFlow中整个画布对应的model
        var minimapViewMap = this.__lfMap.viewMap;//当前缩略图里的viewMap
        try {
    ...
    MiniMap.prototype.__createViewPort = function () {
        ...
        div.addEventListener('mousedown', this.__startDrag);//在视窗上加上了响应事件
        this.__viewport = div;
    };
